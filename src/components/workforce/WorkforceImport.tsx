...
interface WorkforceImportProps {
  onClose: () => void;
}

const WorkforceImport: React.FC<WorkforceImportProps> = ({ onClose }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { importWorkforceData } = useWorkforceStore();
  ...
  const processExcelData = (data: any[]) => {
    try {
      return data.map(row => {
        ...
        if (!row['Name'] && !row['name']) throw new Error('Name is required');
        if (!row['Role'] && !row['role']) throw new Error('Role is required');
        if (!row['Workload'] && !row['workload']) throw new Error('Workload is required');
        if (!row['Tasks'] && !row['tasks']) throw new Error('Tasks count is required');

        return {
          name: row['Name'] || row['name'],
          role: row['Role'] || row['role'],
          workload: parseInt(row['Workload'] || row['workload']),
          tasks: parseInt(row['Tasks'] || row['tasks']),
  ...
  const handleFile = async (file: File) => {
    try {
      ...
      importWorkforceData(processedData);
      
      addNotification({
        title: 'Data Import Successful',
        message: `Successfully imported ${processedData.length} workforce records`,
        type: 'success',
        priority: 'medium'
      });
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 w-full max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">Import Workforce Data</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Upload an Excel or CSV file containing workforce data with the following columns:
          </p>
          <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-md">
            ...
            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              <li>Name (required)</li>
              <li>Role (required)</li>
              <li>Workload (required, number)</li>
              <li>Tasks (required, number)</li>
              <li>Avatar (optional, URL)</li>
              <li>Trend (optional: increasing/decreasing/stable)</li>
              <li>Recommendations (optional, semicolon-separated)</li>
            </ul>
          </div>
        </div>
